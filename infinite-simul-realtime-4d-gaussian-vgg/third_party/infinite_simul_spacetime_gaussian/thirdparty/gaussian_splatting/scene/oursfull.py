...
class GaussianModel:
    ...
    def save_ply(self, path):
        ...
        f_dc_raw = self._features_dc.detach().cpu().numpy()
        # Convert albedo channels (first 3) from logit space to [0,1] via sigmoid
        # so standard splat viewers display correct colors
        f_dc = f_dc_raw.copy()
        f_dc[:, :3] = 1.0 / (1.0 + np.exp(-f_dc_raw[:, :3]))
    ...
    def load_plyandminmax(self, path,  maxx, maxy, maxz,  minx, miny, minz):
        ...
        # Convert albedo channels (first 3) back from [0,1] to logit space for training
        features_dc[mask, :3] = np.clip(features_dc[mask, :3], 1e-6, 1.0 - 1e-6)
        features_dc[mask, :3] = np.log(features_dc[mask, :3] / (1.0 - features_dc[mask, :3]))
    ...
    def load_plyandminmaxall(self, path,  maxx, maxy, maxz,  minx, miny, minz):
        ...
        # Convert loaded albedo channels (first 3) from [0,1] back to logit space for training
        features_dc[mask, :3] = np.clip(features_dc[mask, :3], 1e-6, 1.0 - 1e-6)
        features_dc[mask, :3] = np.log(features_dc[mask, :3] / (1.0 - features_dc[mask, :3]))
    ...
    def load_ply(self, path):
        ...
        if self.rgbfunc == "sandwichnoactss" or self.rgbfunc == 'sandwichnoact' or self.rgbfunc == 'sandwich':
            self.rgbdecoder.load_state_dict(ckpt)
        ...
        # Convert albedo channels (first 3) back from [0,1] to logit space for training
        features_dc[:, :3] = np.clip(features_dc[:, :3], 1e-6, 1.0 - 1e-6)
        features_dc[:, :3] = np.log(features_dc[:, :3] / (1.0 - features_dc[:, :3]))

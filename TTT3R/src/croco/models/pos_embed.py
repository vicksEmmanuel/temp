...
try:
    ...
    class RoPE2D(torch.nn.Module):
        ...
        def apply_rope1d(self, tokens, pos1d, cos, sin):
            assert pos1d.ndim == 2
            # Ensure indices are non-negative to avoid out-of-bounds errors in embedding
            safe_pos1d = pos1d.clamp(min=0)
        ...
        def forward(self, tokens, positions):
            ...
            # Clamp positions to 0 before taking max to handle any legacy negative indices safely
            max_pos = int(positions.clamp(min=0).max()) + 1

...
def transpose_to_landscape(head, activate=True):
    ...
    def wrapper_yes(decout, true_shape):
        ...
        if is_landscape.all():
            return head(decout, (H, W))
        if is_portrait.all():
            return transposed(head(transposed(decout), (W, H)))
        # batch is a mix of both portraint & landscape
        def selout(ar): return [d[ar] for d in decout]

        l_result = head(selout(is_landscape), (H, W))
        p_result = transposed(head(transposed(selout(is_portrait)), (W, H)))
...
def transposed(dic):
    if type(dic) == dict:
        return {k: v.transpose(1, 2) for k, v in dic.items()}
    if type(dic) == list:
        ret_list = []
        for d in dic:
            if type(d) == torch.Tensor and d.ndim == 4:
                ret_list.append(d.transpose(1, 2))
            else:
                ret_list.append(d)
        return ret_list
    else:
        return dic.transpose(1, 2)

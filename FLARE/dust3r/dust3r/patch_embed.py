...
def get_patch_embed(patch_embed_cls, img_size, patch_size, enc_embed_dim, input_dim=3):
    assert patch_embed_cls in ['PatchEmbedDust3R', 'ManyAR_PatchEmbed']
    patch_embed = eval(patch_embed_cls)(img_size, patch_size, input_dim, enc_embed_dim)
    return patch_embed
...
class ManyAR_PatchEmbed (PatchEmbed):
    ...
    def forward(self, img, true_shape):
        ...
        x[is_landscape] = self.proj(img[is_landscape]).permute(0, 2, 3, 1).flatten(1, 2).to(x.dtype)
        x[is_portrait] = self.proj(img[is_portrait].swapaxes(-1, -2)).permute(0, 2, 3, 1).flatten(1, 2).to(x.dtype)

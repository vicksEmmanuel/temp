...
def rescale_image_depthmap(image, depthmap, camera_intrinsics, output_resolution, force=True, depth_anything=None):
    ...
    if depth_anything is not None:
        assert tuple(depth_anything.shape[:2]) == image.size[::-1]
    ...
    if scale_final >= 1 and not force:  # image is already smaller than what is asked
        if depth_anything is None:
            ...
            return (image.to_pil(), depthmap, camera_intrinsics, depth_anything)
    ...
    if depth_anything is not None:
        depth_anything = cv2.resize(depth_anything, output_resolution, fx=scale_final,
                                    fy=scale_final, interpolation=cv2.INTER_NEAREST)
...
def crop_image_depthmap(image, depthmap, camera_intrinsics, crop_bbox, depth_anything=None):
    """
    ...
    if depth_anything is None:
        ...
        depth_anything = depth_anything[t:b, l:r]
        return image.to_pil(), depthmap, camera_intrinsics, depth_anything

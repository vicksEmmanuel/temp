...
def pose_encoding_to_camera(
    pose_encoding,
    pose_encoding_type="vgg",
    log_focal_length_bias=1.,
    ...
    if pose_encoding_type == "train":
        ...
        # Now converted back
        m = nn.Softplus()
        log_focal_length = m(log_focal_length)
        # clamp to avoid weird fl values
        focal_length = torch.clamp(log_focal_length, min=min_focal_length, max=max_focal_length)
    elif pose_encoding_type == "train_fusion":
        # 3 for absT, 4 for quaR, 2 for absF
        abs_T = pose_encoding_reshaped[:, :3]
        quaternion_R = pose_encoding_reshaped[:, 3:7]
        R = quaternion_to_matrix(quaternion_R)
        scale = pose_encoding_reshaped[:, 7:8]
        # log_focal_length_bias was the hyperparameter
        # to ensure the mean of logFL close to 0 during training
        # Now converted back
        focal_length = softplus(scale)
        # clamp to avoid weird fl values
    ...
    # if return_dict:
    return {"focal_length": focal_length, "R": R, "T": abs_T, "quaternion_R": quaternion_R}
...
def get_2d_sincos_pos_embed(embed_dim: int, pos_encoding, return_grid=False) -> torch.Tensor:
    """
    ...
    grid = pos_encoding  # grid.reshape([..., 2])
    pos_embed = get_2d_sincos_pos_embed_from_grid(embed_dim, grid)
    return pos_embed.reshape(pos_encoding.shape[0], pos_encoding.shape[1], -1)
...
def get_2d_sincos_pos_embed_from_grid(embed_dim: int, grid: torch.Tensor) -> torch.Tensor:
    ...
    emb_h = get_1d_sincos_pos_embed_from_grid(embed_dim // 2, grid[..., 0].detach().cpu())  # (H*W, D/2)
    emb_w = get_1d_sincos_pos_embed_from_grid(embed_dim // 2, grid[..., 1].detach().cpu())  # (H*W, D/2)
